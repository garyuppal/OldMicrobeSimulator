#ifndef MICROBE_SIMULATOR_SIMULATOR_H
#define MICROBE_SIMULATOR_SIMULATOR_H
...
namespace MicrobeSimulator{
	...
	template <int dim>
	class GaussianFE : public Function<dim>
	{
	public:
		GaussianFE() {}
		GaussianFE(const Point<dim> &p) : center(p) {}

		virtual double value(const Point<dim> &p, const unsigned int /*component = 0*/) const
		{
			return exp( -(p-center)*(p-center) ); // exp[ -(x-p)^2 ]
		}
	...
	private:
    	...
    	// SparseMatrix<double> rhs_matrix1;
	    // SparseMatrix<double> rhs_matrix2;
	    ...
	    Vector<double>       tmp1, tmp2; // to store rhs
	    Vector<double>       temporary_one, temporary_two;
...
// ----------------------------------------------------------------------------------------
	...
	template<int dim>
	void Simulator<dim>::setup_system(const ArgParser& systemParameters)
	{
	...
	template<int dim>
	void Simulator<dim>::initialize_base_objects(const ArgParser& systemParameters)
	{
		time_step = systemParameters.getTimeStep();
		run_time = systemParameters.getRunTime();
		save_period = systemParameters.getSavePeriod();
		diffusion_constant1 = systemParameters.getGoodSecretionRate();
		diffusion_constant2 = systemParameters.getWasteSecretionRate();
	...
	template<int dim>
	void Simulator<dim>::setup_constraints()
	{
...
//		std::array<BoundaryCondition, dim> boundaryConditions = geometry.getBoundaryConditions();
	...
	template<int dim>
	void Simulator<dim>::initialize_vectors_matrices()
	{
	...
	template<int dim>
	std::vector<Point<dim> > Simulator<dim>::getBacteriaLocations(unsigned int number_bacteria, 
		unsigned int number_groups)
	{
	...
	template<int dim>
	std::vector<Point<dim> > Simulator<dim>::getMixerLocations(unsigned int number_groups)
	{
		if(dim != 2)
	...
	template<int dim>
	void Simulator<dim>::create_advection_matrix()
	{
    ...
    template<int dim>
    void Simulator<dim>::solve_time_step()
    {
    ...
    template<int dim>
    void Simulator<dim>::adv_solve_time_step()
    {
		SolverControl solver_control(1000, 1e-8 * system_rhs1.l2_norm());
	...
	template<int dim>
	void Simulator<dim>::update_chemicals()
	{
		if(advectionField.getVelocityType()!= VelocityType::NO_FLOW)
			solve_time_step();
		else
    ...
    template<int dim>
    void Simulator<dim>::output_results() const
    {
	...
	template<int dim>    
    void Simulator<dim>::output_bacteria() const
    {
    ...
    template<int dim>
    void Simulator<dim>::createGrid(unsigned int global_refinement, 
    	unsigned int sphere_refinement)
    {
    ...
    template<int dim>
    void Simulator<dim>::refineMeshSpheres(unsigned int sphere_refinement)
    {
	  // for each circle:
    ...
    template<int dim>
    void Simulator<dim>::updateMatricesWithPointSources()
    {
		...
		system_matrix1.add(time_step * diffusion_constant1, laplace_matrix);   //  for diffusion term -> LHS = (1+k \lam) M + kd*L
		...
		system_matrix2.add(time_step * diffusion_constant2, laplace_matrix);
    ...
    template<int dim>
	void Simulator<dim>::updatePointSources()
	{
...
//----------------------------------------------------------------------------------------------------
	template<int dim>
	void Simulator<dim>::run(const ArgParser& systemParameters)
	{
		// setup system with parameters:
		setup_system(systemParameters); 
	
		std::cout << "\n...Running FEM (CG) simulator in " 
			<< dim << " dimensions...\n" << std::endl;
...
//----------------------------------------------------------------------------------------------------
	...
	template<int dim>
    void Simulator<dim>::create_mass_check_vector()
	{
    ...
    template<int dim>
	void Simulator<dim>::initialize_base_objects_debug(const ArgParser& systemParameters)
	{
		time_step = systemParameters.getTimeStep();
		run_time = systemParameters.getRunTime();
		save_period = systemParameters.getSavePeriod();
	...
	template<int dim>
	void Simulator<dim>::setup_system_debug(const ArgParser& systemParameters)
	{
	...
	template<int dim>
	void Simulator<dim>::printMassToFile(std::vector<double> massOne, 
		std::vector<double> massTwo) const
	{
	...
	template<int dim>
	void Simulator<dim>::projectInitialCondition(Point<dim> gaussian_center)
	{
	...
	template<int dim>
	void Simulator<dim>::updateMatrices(bool usePointSources)
	{
		mass_matrix.vmult(system_rhs1, old_solution1);  // Matrix vector multiplication (MU^{n-1}) -- overwrites system_rhs
		mass_matrix.vmult(system_rhs2, old_solution2);
	...
	template<int dim>
	Point<dim> Simulator<dim>::findGaussianCenterPoint() const
	{
		Point<dim> result;
		// center y and z:
	...
	template<int dim>
	void Simulator<dim>::run_debug(const ArgParser& systemParameters)
	{
